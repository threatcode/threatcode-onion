# Test output backends for threatcodec
...
from threatcode.backends.sqlite import SQLiteBackend

from threatcode.parser.collection import ThreatcodeCollectionParser
from threatcode.config.mapping import FieldMapping
...
class TestFullTextSearch(unittest.TestCase):
    ...
    def validate(self, detection, expectation):
        ...
        with patch("yaml.safe_load_all", return_value=[self.basic_rule]):
            parser = ThreatcodeCollectionParser("any threatcode io", config, None)
