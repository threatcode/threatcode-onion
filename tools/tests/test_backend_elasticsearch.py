from threatcode.backends.elasticsearch import ElasticsearchDSLBackend
...
def test_backend_elastic():
    ...
    """
    threatcode_config = ThreatcodeConfiguration()
    backend = ElasticsearchDSLBackend(threatcode_config)

    # setup the aggregator input object without calling __init__()
    agg = object.__new__(ThreatcodeAggregationParser)
    agg.condition = "3"
    agg.cond_op = "<"
    agg.aggfunc = ThreatcodeAggregationParser.AGGFUNC_COUNT
