#!/usr/bin/env python3
# Assign UUIDs to Threatcode rules and verify UUID assignment for a Threatcode rule repository
...
def main():
    argparser = ArgumentParser(description="Assign and verfify UUIDs of Threatcode rules")
    ...
    argparser.add_argument("inputs", nargs="+", help="Threatcode rule files or repository directories")
        ...
        if args.verify:
            ...
            if changed:
                with path.open("w") as f:
                    yaml.dump_all(newrules, f, Dumper=ThreatcodeYAMLDumper, indent=4, width=160, default_flow_style=False)

    if not passed:
        print("The Threatcode rules listed above don't have an ID. The ID must be:")
        print("* Contained in the 'id' attribute")
        print("* a valid UUIDv4 (randomly generated)")
        print("* Unique in this repository")
        print("Please generate one with the threatcode_uuid tool or here: https://www.uuidgenerator.net/version4")
        if args.error:
            exit(10)
