...
from threatcode.parser.condition import ConditionOR, NodeSubexpression, ConditionNULLValue
from .exceptions import ThreatcodeConfigParseError, FieldMappingError
...
class SimpleFieldMapping:
    ...
    def resolve(self, key, value, threatcodeparser):
        """Return mapped field name"""
        return (self.target, value)

    def resolve_fieldname(self, fieldname, threatcodeparser=None):
        return self.target
...
class ConditionalFieldMapping(SimpleFieldMapping):
    ...
    * if no condition matches and there is no default, don't perform mapping
                ...
                if condition == "default":
                    if self.default == None:
                        if type(target) == str:
                            ...
                            raise ThreatcodeConfigParseError("Default mapping must be single value or list")
                    else:
                        raise ThreatcodeConfigParseError("Conditional field mapping can have only one default value, use list for multiple target mappings")
                else:
                    raise ThreatcodeConfigParseError("Expected condition or default") from e
    ...
    def _targets(self, threatcodeparser):
        # build list of matching target mappings
        targets = set()
        for condfield in self.conditions:
            if condfield in threatcodeparser.values:
                rulefieldvalues = threatcodeparser.values[condfield]
    ...
    def resolve(self, key, value, threatcodeparser):
        targets = self._targets(threatcodeparser)
...
class FieldMappingChain(object):
    ...
    def resolve(self, key, value, threatcodeparser):
        if type(self.fieldmappings) == str:     # one field mapping
            ...
            logsource = threatcodeparser.parsedyaml.get("logsource")
            ...
            return self.fieldmappings.resolve(self.fieldmappings.source, value, threatcodeparser)
        elif isinstance(self.fieldmappings, SimpleFieldMapping):
            return self.fieldmappings.resolve(key, value, threatcodeparser)
            ...
            for mapping in self.fieldmappings:
                if type(mapping) == str:
                    ...
                    cond.add(mapping.resolve(key, value, threatcodeparser))
            return NodeSubexpression(cond)
    ...
    def resolve_fieldname(self, fieldname, threatcodeparser=None):
        if type(self.fieldmappings) == str:     # one field mapping
            ...
            return self.fieldmappings.resolve_fieldname(fieldname, threatcodeparser)
        elif type(self.fieldmappings) == set:
            mappings = set()
            for mapping in self.fieldmappings:
                if type(mapping) == str:
                    ...
                    resolved_mapping = mapping.resolve_fieldname(fieldname, threatcodeparser)
                    if type(resolved_mapping) is list:
