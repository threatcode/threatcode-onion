...
# Rule Filtering
class ThreatcodeRuleFilter:
    """Filter for Threatcode rules with conditions"""
    LEVELS = {
            "low"      : 0,
            "medium"   : 1,
            "high"     : 2,
        ...
        for cond in [c.replace(" ", "") for c in expr.split(",")]:
            if cond.startswith("level<="):
                try:
                    ...
                    raise ThreatcodeRuleFilterParseException("Unknown level '%s' in condition '%s'" % (level, cond)) from e
                ...
                if self.status not in self.STATES:
                    raise ThreatcodeRuleFilterParseException("Unknown status '%s' in condition '%s'" % (self.status, cond))
                ...
                raise ThreatcodeRuleFilterParseException("Unknown condition '%s'" % cond)
...
class ThreatcodeRuleFilterParseException(Exception):
    pass
