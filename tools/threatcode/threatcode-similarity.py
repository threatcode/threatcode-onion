#!/usr/bin/env python3
# Calculates similarity of Threatcode rules by transformation into a normalized
...
argparser = argparse.ArgumentParser(description="Calculate a similarity score between Threatcode rules.")
...
argparser.add_argument("inputs", nargs="+", help="Threatcode input files")
...
class ThreatcodeNormalizationBackend(SingleTextQueryBackend):
    """Normalization of a Threatcode rule into a non-existing query language that supports all Threatcode features"""
    andToken = " AND "
    orToken = " OR "
    notToken = " NOT "
    subExpression = "(%s)"
...
backend = ThreatcodeNormalizationBackend(ThreatcodeConfiguration())
...
parsed = {
            str(path): ThreatcodeCollectionParser(path.open().read())
            for path in paths
        }
converted = {
            str(path): list(threatcode_collection.generate(backend))
            for path, threatcode_collection in parsed.items()
        }
converted_flat = (
            (path, i, normalized)
