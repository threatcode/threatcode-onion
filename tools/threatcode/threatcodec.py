#!/usr/bin/env python3
# A Threatcode to SIEM converter
...
from threatcode.parser.exceptions import ThreatcodeCollectionParseError, ThreatcodeParseError
from threatcode.configuration import ThreatcodeConfiguration, ThreatcodeConfigurationChain
from threatcode.config.collection import ThreatcodeConfigurationManager
from threatcode.config.exceptions import ThreatcodeConfigParseError, ThreatcodeRuleFilterParseException
from threatcode.filter import ThreatcodeRuleFilter
import threatcode.backends.discovery as backends
from threatcode.backends.base import BackendOptions
from threatcode.backends.exceptions import BackendError, NotSupportedError, PartialMatchError, FullMatchError
from threatcode.parser.modifiers import modifiers
import codecs
...
ERR_OUTPUT              = 1
ERR_INVALID_YAML        = 3
ERR_THREATCODE_PARSING       = 4
ERR_OPEN_THREATCODE_RULE     = 5
...
def set_argparser():
    """Sets up and parses the command line arguments for Threatcodec.
    ...
    argparser.add_argument("inputs", nargs="*", help="Threatcode input files ('-' for stdin)")
...
def main():
    ...
    scm = ThreatcodeConfigurationManager()
    ...
    if cmdargs.filter:
        try:
            rulefilter = ThreatcodeRuleFilter(cmdargs.filter)
        except ThreatcodeRuleFilterParseException as e:
            print("Parse error in Threatcode rule filter expression: %s" % str(e), file=sys.stderr)
            sys.exit(ERR_RULE_FILTER_PARSING)

    threatcodeconfigs = ThreatcodeConfigurationChain()
    ...
    if cmdargs.config:
        ...
        for conf_name in cmdargs.config:
            try:
                threatcodeconfig = scm.get(conf_name)
                if threatcodeconfig.order is not None:
                    if threatcodeconfig.order <= order and not cmdargs.shoot_yourself_in_the_foot:
                    ...
                    order = threatcodeconfig.order

                try:
                    if cmdargs.target not in threatcodeconfig.config["backends"]:
                        print("The configuration '{}' is not valid for backend '{}'. Valid choices are: {}".format(conf_name, cmdargs.target, ", ".join(threatcodeconfig.config["backends"])), file=sys.stderr)
                        sys.exit(ERR_CONFIG_ORDER)
                except KeyError:
                    pass

                threatcodeconfigs.append(threatcodeconfig)
            except OSError as e:
                print("Failed to open Threatcode configuration file %s: %s" % (conf_name, str(e)), file=sys.stderr)
                exit(ERR_OPEN_CONFIG_FILE)
            except (yaml.parser.ParserError, yaml.scanner.ScannerError) as e:
                print("Threatcode configuration file %s is no valid YAML: %s" % (conf_name, str(e)), file=sys.stderr)
                exit(ERR_CONFIG_INVALID_YAML)
            except ThreatcodeConfigParseError as e:
                print("Threatcode configuration parse error in %s: %s" % (conf_name, str(e)), file=sys.stderr)
                exit(ERR_CONFIG_PARSING)

    backend_options = BackendOptions(cmdargs.backend_option, cmdargs.backend_config)
    backend = backend_class(threatcodeconfigs, backend_options)
    ...
    error = 0
    for threatcodefile in get_inputs(cmdargs.inputs, cmdargs.recurse):
        logger.debug("* Processing Threatcode input %s" % (threatcodefile))
        try:
            if cmdargs.inputs == ['-']:
                f = threatcodefile
            else:
                f = threatcodefile.open(encoding='utf-8')
            parser = ThreatcodeCollectionParser(f, threatcodeconfigs, rulefilter)
            results = parser.generate(backend)
            for result in results:
                print(result, file=out)
        except OSError as e:
            print("Failed to open Threatcode file %s: %s" % (threatcodefile, str(e)), file=sys.stderr)
            error = ERR_OPEN_THREATCODE_RULE
        except (yaml.parser.ParserError, yaml.scanner.ScannerError) as e:
            print("Threatcode file %s is no valid YAML: %s" % (threatcodefile, str(e)), file=sys.stderr)
            error = ERR_INVALID_YAML
            if not cmdargs.defer_abort:
                sys.exit(error)
        except (ThreatcodeParseError, ThreatcodeCollectionParseError) as e:
            print("Threatcode parse error in %s: %s" % (threatcodefile, str(e)), file=sys.stderr)
            error = ERR_THREATCODE_PARSING
            if not cmdargs.defer_abort:
                sys.exit(error)
        except NotSupportedError as e:
            print("The Threatcode rule requires a feature that is not supported by the target system: " + str(e), file=sys.stderr)
            if not cmdargs.ignore_backend_errors:
                error = ERR_NOT_SUPPORTED
                if not cmdargs.defer_abort:
                    sys.exit(error)
        except BackendError as e:
            print("Backend error in %s: %s" % (threatcodefile, str(e)), file=sys.stderr)
            if not cmdargs.ignore_backend_errors:
                error = ERR_BACKEND
                if not cmdargs.defer_abort:
                    sys.exit(error)
        except (NotImplementedError, TypeError) as e:
            print("An unsupported feature is required for this Threatcode rule (%s): " % (threatcodefile) + str(e), file=sys.stderr)
            print("Feel free to contribute for fun and fame, this is open source :) -> https://github.com/threatcode/threatcode", file=sys.stderr)
            if not cmdargs.ignore_backend_errors:
