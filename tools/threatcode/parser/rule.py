# Threatcode parser
# Copyright 2016-2019 Md Sulaiman, Florian Roth
...
from .condition import ThreatcodeConditionTokenizer, ThreatcodeConditionParser, ConditionAND, ConditionOR, ConditionNULLValue
from .modifiers import apply_modifiers

class ThreatcodeParser:
    """Parse a Threatcode rule (definitions, conditions and aggregations)"""
    def __init__(self, threatcode, config):
        self.definitions = dict()
        self.values = dict()
        self.config = config
        self.parsedyaml = threatcode
        self.parse_threatcode()

    def parse_threatcode(self):
        try:    # definition uniqueness check
            ...
            raise ThreatcodeParseError("No detection definitions found")

        try:    # tokenization
            conditions = self.parsedyaml["detection"]["condition"]
            self.condtoken = list()     # list of tokenized conditions
            if type(conditions) == str:
                self.condtoken.append(ThreatcodeConditionTokenizer(conditions))
            elif type(conditions) == list:
                for condition in conditions:
                    self.condtoken.append(ThreatcodeConditionTokenizer(condition))
        except KeyError:
            raise ThreatcodeParseError("No condition found")

        self.condparsed = list()        # list of parsed conditions
        for tokens in self.condtoken:
            condparsed = ThreatcodeConditionParser(self, tokens)
    ...
    def parse_definition_byname(self, definitionName, condOverride=None):
        try:
            ...
            raise ThreatcodeParseError("Unknown definition '%s'" % definitionName) from e
    ...
    def parse_definition(self, definition, condOverride=None):
        if type(definition) not in (dict, list):
            raise ThreatcodeParseError("Expected map or list, got type %s: '%s'" % (type(definition), str(definition)))
        ...
        if type(definition) == list:    # list of values or maps
            ...
            for value in definition:
                if type(value) in (str, int):
                    ...
                    raise ThreatcodeParseError("Definition list may only contain plain values or maps")
