# Threatcode parser
# Copyright 2016-2017 Md Sulaiman, Florian Roth
...
from .exceptions import ThreatcodeParseError
...
# Condition Tokenizer
class ThreatcodeConditionToken:
    """Token of a Threatcode condition expression"""
    TOKEN_AND    = 1
    TOKEN_OR     = 2
    TOKEN_NOT    = 3
    TOKEN_ID     = 4
    ...
    def __eq__(self, other):
        ...
        if type(other) == str:      # match against content
            ...
            raise NotImplementedError("ThreatcodeConditionToken can only be compared against token type constants")

    def __str__(self):  # pragma: no cover
        return "[ Token: %s: '%s' ]" % (self.tokenstr[self.type], self.matched)


class ThreatcodeConditionTokenizer:
    ...
    tokendefs = [      # list of tokens, preferred recognition in given order, (token identifier, matching regular expression). Ignored if token id == None
            (ThreatcodeConditionToken.TOKEN_ONE,    re.compile("1 of", re.IGNORECASE)),
            (ThreatcodeConditionToken.TOKEN_ALL,    re.compile("all of", re.IGNORECASE)),
            (None,       re.compile("[\\s\\r\\n]+")),
            (ThreatcodeConditionToken.TOKEN_AGG,    re.compile("count|min|max|avg|sum", re.IGNORECASE)),
            (ThreatcodeConditionToken.TOKEN_NEAR,   re.compile("near", re.IGNORECASE)),
            (ThreatcodeConditionToken.TOKEN_BY,     re.compile("by", re.IGNORECASE)),
            (ThreatcodeConditionToken.TOKEN_EQ,     re.compile("==")),
            (ThreatcodeConditionToken.TOKEN_LT,     re.compile("<")),
            (ThreatcodeConditionToken.TOKEN_LTE,    re.compile("<=")),
            (ThreatcodeConditionToken.TOKEN_GT,     re.compile(">")),
            (ThreatcodeConditionToken.TOKEN_GTE,    re.compile(">=")),
            (ThreatcodeConditionToken.TOKEN_PIPE,   re.compile("\\|")),
            (ThreatcodeConditionToken.TOKEN_AND,    re.compile("and", re.IGNORECASE)),
            (ThreatcodeConditionToken.TOKEN_OR,     re.compile("or", re.IGNORECASE)),
            (ThreatcodeConditionToken.TOKEN_NOT,    re.compile("not", re.IGNORECASE)),
            (ThreatcodeConditionToken.TOKEN_ID,     re.compile("[\\w*]+")),
            (ThreatcodeConditionToken.TOKEN_LPAR,   re.compile("\\(")),
            (ThreatcodeConditionToken.TOKEN_RPAR,   re.compile("\\)")),
            ]
        ...
        if type(condition) == str:          # String that is parsed
            ...
            while len(condition) > 0:
                for tokendef in self.tokendefs:     # iterate over defined tokens and try to recognize the next one
                    ...
                    if match:
                        if tokendef[0] != None:
                            self.tokens.append(ThreatcodeConditionToken(tokendef, match, pos + match.start()))
                    ...
                    raise ThreatcodeParseError("Unexpected token in condition at position %s" % condition)
        elif type(condition) == list:       # List of tokens to be converted into ThreatcodeConditionTokenizer class
            self.tokens = condition
        else:
            raise TypeError("ThreatcodeConditionTokenizer constructor expects string or list, got %s" % (type(condition)))
    ...
    def __getitem__(self, i):
        if type(i) == int:
            ...
            return ThreatcodeConditionTokenizer(self.tokens[i])
        else:
            raise IndexError("Expected index or slice")

    def __add__(self, other):
        if isinstance(other, ThreatcodeConditionTokenizer):
            return ThreatcodeConditionTokenizer(self.tokens + other.tokens)
        elif isinstance(other, (ThreatcodeConditionToken, ParseTreeNode)):
            return ThreatcodeConditionTokenizer(self.tokens + [ other ])
        else:
            raise TypeError("+ operator expects ThreatcodeConditionTokenizer or token type, got %s: %s" % (type(other), str(other)))
...
class ConditionAND(ConditionBase):
    ...
    def __init__(self, threatcode=None, op=None, *args):
        if threatcode == None and op == None and len(args) == 0:    # no parameters given - initialize empty
            self.items = list()
...
class ConditionNOT(ConditionBase):
    ...
    def __init__(self, threatcode=None, op=None, val=None):
        if threatcode == None and op == None and val == None:    # no parameters given - initialize empty
            self.items = list()
        else:       # called by parser, use given values
            self.items = [ val ]
...
def generateXOf(threatcode, val, condclass):
    """
    ...
    * condclass across all matching definition if x is wildcard expression, e.g. 'selection*'
    ...
    if val.matched == "them":           # OR across all definitions
        cond = condclass()
        for name, definition in threatcode.definitions.items():
            if name == "timeframe":
                continue
            cond.add(NodeSubexpression(threatcode.parse_definition(definition)))
        ...
        return NodeSubexpression(threatcode.parse_definition_byname(val.matched, condclass))


def generateAllOf(threatcode, op, val):
    """Convert 'all of x' expressions into ConditionAND"""
    return generateXOf(threatcode, val, ConditionAND)


def generateOneOf(threatcode, op, val):
    """Convert '1 of x' expressions into ConditionOR"""
    return generateXOf(threatcode, val, ConditionOR)


def convertId(threatcode, op):
    """Convert search identifiers (lists or maps) into condition nodes according to spec defaults"""
    return NodeSubexpression(threatcode.parse_definition_byname(op.matched))


# Optimizer
class ThreatcodeConditionOptimizer:
    """
...
# Condition parser
class ThreatcodeConditionParser:
    """Parser for Threatcode condition expression"""
    searchOperators = [     # description of operators: (token id, number of operands, parse tree node class) - order == precedence
            (ThreatcodeConditionToken.TOKEN_ALL, 1, generateAllOf),
            (ThreatcodeConditionToken.TOKEN_ONE, 1, generateOneOf),
            (ThreatcodeConditionToken.TOKEN_ID,  0, convertId),
            (ThreatcodeConditionToken.TOKEN_NOT, 1, ConditionNOT),
            (ThreatcodeConditionToken.TOKEN_AND, 2, ConditionAND),
            (ThreatcodeConditionToken.TOKEN_OR,  2, ConditionOR),
            ]

    def __init__(self, threatcodeParser, tokens):
        self.threatcodeParser = threatcodeParser
        self.config = threatcodeParser.config
        self._optimizer = ThreatcodeConditionOptimizer()

        if ThreatcodeConditionToken.TOKEN_PIPE in tokens:    # Condition contains atr least one aggregation expression
            pipepos = tokens.index(ThreatcodeConditionToken.TOKEN_PIPE)
            self.parsedSearch = self.parseSearch(tokens[:pipepos])
            self.parsedAgg = ThreatcodeAggregationParser(tokens[pipepos + 1:], self.threatcodeParser, self.config)
        else:
    ...
    def parseSearch(self, tokens):
        ...
        while ThreatcodeConditionToken.TOKEN_LPAR in tokens:
            lPos = tokens.index(ThreatcodeConditionToken.TOKEN_LPAR)
            lTok = tokens[lPos]
            try:
                rPos = find_close_token_index_in_pairs(tokens, lPos+1, ThreatcodeConditionToken.TOKEN_LPAR, ThreatcodeConditionToken.TOKEN_RPAR)
                rTok = tokens[rPos]
            except ValueError as e:
                raise ThreatcodeParseError("Missing matching closing parentheses") from e
            if lPos + 1 == rPos:
                raise ThreatcodeParseError("Empty subexpression at " + str(lTok.pos))
            if lPos > rPos:
                raise ThreatcodeParseError("Closing parentheses at position " + str(rTok.pos) + " precedes opening at position " + str(lTok.pos))
        ...
        for operator in self.searchOperators:
            ...
            while operator[0] in tokens:
                ...
                if operator[1] == 0:    # operator
                    treenode = operator[2](self.threatcodeParser, tok_op)
                    tokens = tokens[:pos_op] + treenode + tokens[pos_op + 1:]
                elif operator[1] == 1:    # operator value
                    pos_val = pos_op + 1
                    tok_val = tokens[pos_val]
                    treenode = operator[2](self.threatcodeParser, tok_op, tok_val)
                    ...
                    treenode = operator[2](self.threatcodeParser, tok_op, tok_val1, tok_val2)
        ...
        ls_cond = self.threatcodeParser.get_logsource_condition()
...
# Aggregation parser
class ThreatcodeAggregationParser(SimpleParser):
    """Parse Threatcode aggregation expression and provide parsed data"""
    parsingrules = [
            {   # State 0
                ThreatcodeConditionToken.TOKEN_AGG:  ("aggfunc", "trans_aggfunc", 1),
                ThreatcodeConditionToken.TOKEN_NEAR: ("aggfunc", "init_near_parsing", 8),
            },
            {   # State 1
                ThreatcodeConditionToken.TOKEN_LPAR: (None, None, 2)
            },
            {   # State 2
                ThreatcodeConditionToken.TOKEN_RPAR: (None, None, 4),
                ThreatcodeConditionToken.TOKEN_ID: ("aggfield", "trans_fieldname", 3),
            },
            {   # State 3
                ThreatcodeConditionToken.TOKEN_RPAR: (None, None, 4)
            },
            {   # State 4
                ThreatcodeConditionToken.TOKEN_BY: ("cond_op", None, 5),
                ThreatcodeConditionToken.TOKEN_EQ: ("cond_op", None, 7),
                ThreatcodeConditionToken.TOKEN_LT: ("cond_op", None, 7),
                ThreatcodeConditionToken.TOKEN_LTE: ("cond_op", None, 7),
                ThreatcodeConditionToken.TOKEN_GT: ("cond_op", None, 7),
                ThreatcodeConditionToken.TOKEN_GTE: ("cond_op", None, 7),
            },
            {   # State 5
                ThreatcodeConditionToken.TOKEN_ID: ("groupfield", "trans_fieldname", 6)
            },
            {   # State 6
                ThreatcodeConditionToken.TOKEN_EQ: ("cond_op", None, 7),
                ThreatcodeConditionToken.TOKEN_LT: ("cond_op", None, 7),
            ...
            {   # State 7
                ThreatcodeConditionToken.TOKEN_ID: ("condition", None, -1)
            },
            {   # State 8
                ThreatcodeConditionToken.TOKEN_ID: (None, "store_search_id", 9)
            },
            {   # State 9
                ThreatcodeConditionToken.TOKEN_AND: (None, "set_include", 10),
            },
            {   # State 10
                ThreatcodeConditionToken.TOKEN_NOT: (None, "set_exclude", 8),
                ThreatcodeConditionToken.TOKEN_ID: (None, "store_search_id", 9),
            },
            ]
    finalstates = {-1, 9}
    ...
    def trans_aggfunc(self, name):
        ...
        try:
            ...
            raise ThreatcodeParseError("Unknown aggregation function '%s'" % (name))
