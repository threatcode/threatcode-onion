...
from .exceptions import ThreatcodeModifierValueTypeError

class ThreatcodeModifierTypes(Enum):
    NONE = auto()
    TRANSFORM = auto()
    TYPE = auto()

class ThreatcodeModifier(object):
    """
    Value modifier base class. There are two modifier types (with separate base classes):

    identifier: identifier string that is used in Threatcode rules to use modifier
    active: boolean if modifier will be found by autodiscovery (and therefore usable)
    modifier_type: type from ThreatcodeModifierTypes enumeration
        * Value transformation modifiers: the value or list of values is transformed to a different
            value or a list of values. Base class: ThreatcodeValueModifier
        * Type modifiers: modify the type of the modifier, e.g. to support regular expressions. The
            backend must handle these modifiers accordingly. Base class: ThreatcodeTypeModifier
    ...
    identifier = "base"
    active = False
    modifier_type = ThreatcodeModifierTypes.NONE
    ...
    def __init__(self, value):
        ...
        if not self.validate():
            raise ThreatcodeModifierValueTypeError
    ...
    def apply(self):
        ...
        * Return an object from threatcode.parser.condition that replaces the value in the condition tree
...
class ThreatcodeTransformModifier(ThreatcodeModifier):
    """Transform a value into a different value or a list of values"""
    identifier = "tansform_base"
    modifier_type = ThreatcodeModifierTypes.TRANSFORM

class ThreatcodeTypeModifier(ThreatcodeModifier):
    """Modify the type of the value. This must be handled by the backend."""
    identifier = "type_base"
    modifier_type = ThreatcodeModifierTypes.TYPE
