...
class SimpleParser:
    ...
    * next state: next parser state
    ...
    def __init__(self, tokens, init_state=0):
        ...
        for token in tokens:
            if self.state < 0:
                raise ThreatcodeParseError("No further token expected, but read %s" % (str(token)))
            try:
                rule = self.parsingrules[self.state][token.type]
            except KeyError as e:
                raise ThreatcodeParseError("Unexpected token %s at %d in aggregation expression" % (str(token), token.pos)) from e
        ...
        if self.state not in self.finalstates:
            raise ThreatcodeParseError("Unexpected end of aggregation expression, state=%d" % (self.state))
