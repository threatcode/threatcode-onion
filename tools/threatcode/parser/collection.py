...
import yaml
from .exceptions import ThreatcodeCollectionParseError
from .rule import ThreatcodeParser

class ThreatcodeCollectionParser:
    """
    Parses a Threatcode file that may contain multiple Threatcode rules as different YAML documents.
    ...
    * repeat: takes attributes from this YAML document, merges into previous rule YAML and regenerates the rule
    ...
    def __init__(self, content, config=None, rulefilter=None):
        if config is None:
            ...
            config = ThreatcodeConfiguration()
        ...
        for yamldoc in self.yamls:
            ...
            if action == "global":
                ...
                if prevrule is None:
                    raise ThreatcodeCollectionParseError("action 'repeat' is only applicable after first valid Threatcode rule")
