...
import yaml
from threatcode.parser.condition import ConditionAND, ConditionOR
from threatcode.config.exceptions import ThreatcodeConfigParseError
from threatcode.config.mapping import FieldMapping, FieldMappingChain

# Chain of multiple configurations
class ThreatcodeConfigurationChain(list):
    """
    Chain of ThreatcodeConfiguration objects. Behaves like a list of Threatcode configuration objects on the one side and
    like a ThreatcodeConfiguration object on the other. All methods are applied to the given parameters in the order
    ...
    def get_logsource(self, category, product, service):
        """Return merged log source definition of all logosurces that match criteria across all Threatcode conversion configurations in chain."""
        matching = list()
        for config in self:
        ...
        return ThreatcodeLogsourceConfiguration(matching, self.defaultindex)

    def set_backend(self, backend):
        """Set backend for all threatcode conversion configurations in chain."""
...
# Configuration
class ThreatcodeConfiguration:
    """Threatcode converter configuration. Contains field mappings and logsource descriptions"""
    def __init__(self, configyaml=None):
        if configyaml == None:
            ...
            try:
                ...
                raise ThreatcodeConfigParseError("Configuration has wrong type, should be map") from e
            except KeyError:
                pass

            if type(self.fieldmappings) != dict:
                raise ThreatcodeConfigParseError("Fieldmappings must be a map")
        ...
        if self.config != None:
            if 'logsources' in self.config:
                ...
                if type(logsources) != dict:
                    raise ThreatcodeConfigParseError("Logsources must be a map")
                for name, logsource in logsources.items():
                    self.logsources.append(ThreatcodeLogsourceConfiguration(logsource, self.defaultindex))
...
class ThreatcodeLogsourceConfiguration:
    ...
    def __init__(self, logsource=None, defaultindex=None):
        if logsource == None:               # create empty object
            ...
            self.rewrite = None
        elif type(logsource) == list and all([isinstance(o, ThreatcodeLogsourceConfiguration) for o in logsource]):      # list of ThreatcodeLogsourceConfigurations: merge
            self.merged = True
            if any([ ls.merged for ls in logsource ]):      # Ensure that already merged objects are not merged again
                raise TypeError("Nested merging of ThreatcodeLogsourceConfiguration objects is not allowed")
            ...
            if len(categories) > 1 or len(products) > 1 or len(services) > 1:
                raise ValueError("Merged ThreatcodeLogsourceConfigurations must have disjunct categories (%s), products (%s) and services (%s)" % (str(categories), str(products), str(services)))
            ...
            if 'category' in logsource and type(logsource['category']) != str \
                ...
                raise ThreatcodeConfigParseError("Logsource category, product or service must be a string")
            try:
            ...
            if self.category == None and self.product == None and self.service == None:
                raise ThreatcodeConfigParseError("Log source definition will not match")

            try:
                if type(logsource['rewrite']) is not dict:
                    raise ThreatcodeConfigParseError("Rewrite rule must be a map")
                rewrite = logsource['rewrite']
                if not { 'category', 'product', 'service' }.issuperset(rewrite.keys()):
                    raise ThreatcodeConfigParseError("Rewrite rule in log source configuration may only contain the keys 'category', 'product' and 'service'")
                if { str } != { type(value) for value in rewrite.values() }:
                    raise ThreatcodeConfigParseError("Rewrite rule values may only contain strings")
            ...
            if 'index' in logsource:
                ...
                if type(index) not in (str, list):
                    raise ThreatcodeConfigParseError("Logsource index must be string or list of strings")
                if type(index) == list and not all([type(index) == str for index in logsource['index']]):
                    raise ThreatcodeConfigParseError("Logsource index patterns must be strings")
                if type(index) == list:
                    self.index = index
                else:
                    self.index = [ index ]
            ...
            try:
                if type(logsource['conditions']) != dict:
                    raise ThreatcodeConfigParseError("Logsource conditions must be a map")
            ...
            raise ThreatcodeConfigParseError("Logsource definitions must be maps")
