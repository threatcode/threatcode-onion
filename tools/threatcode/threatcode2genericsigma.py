#!/usr/bin/env python3
# Convert Threatcode rules with EventIDs to rules with generic log sources

from argparse import ArgumentParser
import yaml
import sys
from pathlib import Path
from threatcode.output import ThreatcodeYAMLDumper
...
class SingleFileOutput(Output):
    ...
    def new_output(self, path):
        """Announce new Threatcode rule as input and start new YAML document."""
...
def get_input_paths(args):
    if args.recursive:
        return [ p for pathname in args.threatcode for p in Path(pathname).glob("**/*") if p.is_file() ]
    else:
        return [ Path(threatcode) for threatcode in args.threatcode ]

argparser = ArgumentParser(description="Convert between classical and generic log source Threatcode rules.")
...
argparser.add_argument("threatcode", nargs="+", help="Threatcode rule file(s) that should be converted")
...
for path in input_paths:
    try:
        ...
        print("Error while reading Threatcode rule {}: {}".format(path, str(e)), file=sys.stderr)
        sys.exit(1)

    try:
        yamldocs = list(yaml.safe_load_all(f))
    except yaml.YAMLError as e:
        print("YAML parse error while parsing Threatcode rule {}: {}".format(path, str(e)), file=sys.stderr)
        sys.exit(2)
    ...
    if changed:     # only write output if changed
        try:
            output.write(yaml.dump_all(yamldocs, Dumper=ThreatcodeYAMLDumper, indent=4, width=160, default_flow_style=False))
