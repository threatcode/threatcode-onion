...
class SQLBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into SQL query"""
    identifier = "sql"
    active = True
    ...
    def __init__(self, threatcodeconfig, table):
        super().__init__(threatcodeconfig)
        self.table = table
    ...
    def generateAggregation(self, agg, where_clausel):
        ...
        if  (agg.aggfunc == ThreatcodeAggregationParser.AGGFUNC_COUNT or
            agg.aggfunc == ThreatcodeAggregationParser.AGGFUNC_MAX or
            agg.aggfunc == ThreatcodeAggregationParser.AGGFUNC_MIN or
            agg.aggfunc == ThreatcodeAggregationParser.AGGFUNC_SUM or
            agg.aggfunc == ThreatcodeAggregationParser.AGGFUNC_AVG):
            ...
            if agg.aggfield:
                ...
                if agg.aggfunc == ThreatcodeAggregationParser.AGGFUNC_COUNT:
                    select = "{}(*) AS agg".format(agg.aggfunc_notrans)
                else:
                    raise ThreatcodeParseError("For {} aggregation a fieldname needs to be specified".format(agg.aggfunc_notrans))
