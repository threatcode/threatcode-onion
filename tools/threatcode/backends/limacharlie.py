# LimaCharlie backend for threatcodec created by LimaCharlie.io
...
ThreatcodeLCConfig = namedtuple('ThreatcodeLCConfig', [
    'topLevelParams',
    'preConditions',
    'fieldMappings',
    'isAllStringValues',
...
_allFieldMappings = {
    "windows/process_creation/": ThreatcodeLCConfig(
        topLevelParams = {
            "events": [
                "NEW_PROCESS",
                "EXISTING_PROCESS",
    ...
    "windows_defender//": ThreatcodeLCConfig(
        topLevelParams = {
            "target": "log",
            "log type": "wel",
        },
...
class LimaCharlieBackend(BaseBackend):
    """Converts Threatcode rule into LimaCharlie D&R rules. Contributed by LimaCharlie. https://limacharlie.io"""
    ...
    def generate(self, threatcodeparser):
        ...
        ruleConfig = threatcodeparser.parsedyaml
        ...
        detectComponent = super().generate(threatcodeparser)
    ...
    def generateMapItemNode(self, node):
        ...
        if isinstance(value, (int, str)):
            ...
            if isinstance(value, ThreatcodeRegularExpressionModifier):
                if fieldNameAndValCallback is not None:
                    fieldname, value = fieldNameAndValCallback(fieldname, value)
                result = {
                    "op": "matches",
    ...
    def _valuePatternToLcOp(self, val):
        # Here we convert the string values supported by Threatcode that
