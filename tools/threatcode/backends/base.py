...
class BaseBackend:
    ...
    def __init__(self, threatcodeconfig, backend_options=dict()):
        """
        Initialize backend. This gets a threatcodeconfig object, which is notified about the used backend class by
        passing the object instance to it.
        """
        super().__init__()
        if not isinstance(threatcodeconfig, (threatcode.configuration.ThreatcodeConfiguration, threatcode.configuration.ThreatcodeConfigurationChain, None)):
            raise TypeError("ThreatcodeConfiguration object expected")
        self.backend_options = backend_options
        self.threatcodeconfig = threatcodeconfig
        self.threatcodeconfig.set_backend(self)
    ...
    def applyOverrides(self, query):
        try:
            if 'overrides' in self.threatcodeconfig.config and isinstance(query, str):
                for expression in self.threatcodeconfig.config['overrides']:
    ...
    def generateNode(self, node):
        ...
        elif isinstance(node, ThreatcodeTypeModifier):
...
class SingleTextQueryBackend(RulenameCommentMixin, BaseBackend, QuoteCharMixin):
    """Base class for backends that generate one text-based expression from a Threatcode rule"""
    ...
    def fieldNameMapping(self, fieldname, value):
        ...
        transformed from the original name given in the Threatcode rule.
        """
        return fieldname
