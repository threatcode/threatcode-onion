...
from threatcode.backends.mixins import MultiRuleOutputMixin
...
class SysmonConfigBackend(SingleTextQueryBackend, MultiRuleOutputMixin):
    ...
    def generate(self, threatcodeparser):
        sysmon_rule = None
        title = threatcodeparser.parsedyaml.get("title", "")
        author = threatcodeparser.parsedyaml.get("author", {})
        self.rule_name = "{} by {}".format(title, author)
        self.detection = threatcodeparser.parsedyaml.get("detection", {})
        self.checkDetection()
        self.logsource = threatcodeparser.parsedyaml["logsource"]
        self.createTableFromLogsource()
        rule_type, conditions = self.checkRuleCondition(threatcodeparser.condtoken)
        if rule_type == "single":
