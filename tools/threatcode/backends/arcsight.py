# ArcSight backend for threatcodec created by SOC Prime
...
import re
from threatcode.parser.condition import ConditionOR
from .base import SingleTextQueryBackend

class ArcSightBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into ArcSight saved search. Contributed by SOC Prime. https://socprime.com"""
    identifier = "arcsight"
    active = True
    andToken = " AND "
    orToken = " OR "
    ...
    def generate(self, threatcodeparser):
        ...
        const_title = ' AND type != 2 | rex field = flexString1 mode=sed "s//Threatcode: {}/g"'
        for parsed in threatcodeparser.condparsed:
            return self.generateQuery(parsed) + const_title.format(threatcodeparser.parsedyaml["title"])
...
class ArcSightESMBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into ArcSight ESM saved search. Contributed by SOC Prime. https://socprime.com"""
    reEscape = re.compile('(["\\\()])')
