# Azure Log Analytics output backend for threatcodec
...
from threatcode.config.mapping import (
    SimpleFieldMapping, MultiFieldMapping, ConditionalFieldMapping
)
from threatcode.parser.condition import ThreatcodeAggregationParser
...
class AzureLogAnalyticsBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into Azure Log Analytics Queries."""
    identifier = "ala"
    active = True
    ...
    typedValueExpression = {
        ThreatcodeRegularExpressionModifier: "matches regex \"(?i)%s\"",
        ThreatcodeContainsModifier: "contains \"%s\""
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.sysmon and not self.threatcodeconfig.config:
    ...
    def getTable(self, threatcodeparser):
        if self.category == "process_creation" and len(set(threatcodeparser.values.keys()) - {"Image", "ParentImage",
                                                                                         "CommandLine"}) == 0:
    ...
    def generate(self, threatcodeparser):
        try:
            self.category = threatcodeparser.parsedyaml['logsource'].setdefault('category', None)
            self.product = threatcodeparser.parsedyaml['logsource'].setdefault('product', None)
            self.service = threatcodeparser.parsedyaml['logsource'].setdefault('service', None)
        except KeyError:
            self.category = None
            self.product = None
            self.service = None
        detection = threatcodeparser.parsedyaml.get("detection", {})
        if "keywords" in detection.keys():
            return super().generate(threatcodeparser)
        if self.table is None:
            self.getTable(threatcodeparser)

        return super().generate(threatcodeparser)

    def generateBefore(self, parsed):
        if self.table is None:
            raise NotSupportedError("No table could be determined from Threatcode rule")
    ...
    def generateMapItemNode(self, node):
        ...
        elif type(value) in [ThreatcodeTypeModifier, ThreatcodeContainsModifier, ThreatcodeRegularExpressionModifier, ThreatcodeStartswithModifier, ThreatcodeEndswithModifier]:
    ...
    def generateAggregation(self, agg):
        if agg is None:
            return ""
        if agg.aggfunc == ThreatcodeAggregationParser.AGGFUNC_NEAR:
    ...
    def _map_conditional_field(self, fieldname):
        mapping = self.threatcodeconfig.fieldmappings.get(fieldname)
...
class AzureAPIBackend(AzureLogAnalyticsBackend):
    """Converts Threatcode rule into Azure Log Analytics Rule."""
    identifier = "ala-rule"
    active = True
    options = SingleTextQueryBackend.options + (
            ("sysmon", False, "Generate Sysmon event queries for generic rules", None),
