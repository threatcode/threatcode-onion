# ee-outliers backend for threatcodec
# NVISO (@NVISO_Labs)
...
class OutliersBackend(ElasticsearchDSLBackend, MultiRuleOutputMixin):
    ...
    def generate(self, threatcodeparser):
        super().generate(threatcodeparser)

        self.tags = threatcodeparser.parsedyaml.setdefault("tags", "")
        ...
        use_case_name = self.getRuleName(threatcodeparser)

        index = ''
        if self.indices is not None and len(self.indices) == 1:
            index = self.indices[0]

        types = ["Threatcode hit"]
        ...
        config_data = {
            ...
            "outlier_reason": "Threatcode hit - " + self.title,
            "outlier_summary": "Threatcode hit - " + self.title,
            "run_model": 1,
            "test_model": 0
        }

        config = configparser.ConfigParser(interpolation=None)
        config["simplequery_threatcode_" + use_case_name] = config_data
