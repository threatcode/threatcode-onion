...
from threatcode.parser.modifiers.transform import (
...
class FireEyeHelixBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into FireEye Helix Query Language."""
    ...
    typedValueExpression = {
        ...
        ThreatcodeContainsModifier: "%s",
        ThreatcodeStartswithModifier: "%s",
        ThreatcodeEndswithModifier: "%s",
    }
    ...
    def generateMapItemNode(self, node):
        ...
        if isinstance(value, int):
            ...
            if isinstance(value, (ThreatcodeStartswithModifier, ThreatcodeEndswithModifier)):
                divinedExpression = self.mapContainsExpression
                # Strip prefix/suffix matching wildcards on rawmsg field searches
                if key == self.nonTaxonomyField and isinstance(value, str):
                    value.strip("*")
            elif isinstance(value, ThreatcodeContainsModifier):
                divinedExpression = self.mapContainsExpression
