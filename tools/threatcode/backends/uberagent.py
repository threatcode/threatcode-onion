...
from threatcode.parser.condition import ThreatcodeAggregationParser, NodeSubexpression, ConditionAND, ConditionOR, ConditionNOT
from threatcode.parser.exceptions import ThreatcodeParseError
from .mixins import MultiRuleOutputMixin
from threatcode.parser.modifiers.transform import ThreatcodeContainsModifier, ThreatcodeStartswithModifier, ThreatcodeEndswithModifier
from threatcode.parser.modifiers.type import ThreatcodeRegularExpressionModifier
from ..parser.modifiers.base import ThreatcodeTypeModifier


def convert_threatcode_level_to_uberagent_risk_score(level):
    """Converts the given Threatcode rule level to uberAgent ESA RiskScore property."""
    levels = {
        "critical": 100,
        "high": 75,
        "medium": 50,
...
def convert_threatcode_name_to_uberagent_tag(name):
    """Converts the given Threatcode rule name to uberAgent ESA Tag property."""
    tag = name.lower().replace(" ", "-")
    tag = re.sub(r"-{2,}", "-", tag, 0, re.IGNORECASE)
    return tag


def convert_threatcode_category_to_uberagent_event_type(category):
    categories = {
...
def is_threatcode_category_supported(category):
    """Returns whether uberAgent ESA knows the given category or not."""
    return convert_threatcode_category_to_uberagent_event_type(category) is not None
...
class ActivityMonitoringRule:
    ...
    def set_threatcode_level(self, level):
        """Sets the Threatcode rule level."""
        self.threatcode_level = level
...
def get_parser_properties(threatcodeparser):
    title = threatcodeparser.parsedyaml['title']
    level = threatcodeparser.parsedyaml['level']
    description = threatcodeparser.parsedyaml['description']
    condition = threatcodeparser.parsedyaml['detection']['condition']
    logsource = threatcodeparser.parsedyaml['logsource']
    category = ''
...
def write_file_header(f, level):
    f.write("#\n")
    f.write("# The rules are generated from the Threatcode GitHub repository at https://github.com/threatcode/threatcode\n")
    ...
    f.write("#    3. Run threatcodec -I --target uberagent -r rules/\n")
    f.write("#\n")
    f.write("# The rules in this file are marked with threatcode-level: {}\n".format(level))
    f.write("#\n\n")


class uberAgentBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into uberAgent ESA's process tagging rules."""
    ...
    def generate(self, threatcodeparser):
        ...
        product, category, service, title, level, condition, description = get_parser_properties(threatcodeparser)
        if product not in ["windows"]:
            return ""

        # Do not generate a rule if the given category is unsupported by now.
        if not is_threatcode_category_supported(category):
            return ""
        ...
        try:
            ...
            query = super().generate(threatcodeparser)
            if len(query) > 0:
                rule.set_name(title)
                rule.set_tag(convert_threatcode_name_to_uberagent_tag(title))
                rule.set_event_type(convert_threatcode_category_to_uberagent_event_type(category))
                rule.set_query(query)
                rule.set_risk_score(convert_threatcode_level_to_uberagent_risk_score(level))
                rule.set_threatcode_level(level)
        ...
        with open(name, "w") as file:
            ...
            for rule in self.rules:
                try:
                    ...
                    if rule.threatcode_level == level:
    ...
    def finalize(self):
        count_critical = self.serialize_file("uberAgent-ESA-am-threatcode-proc-creation-critical.conf", "critical")
        count_high = self.serialize_file("uberAgent-ESA-am-threatcode-proc-creation-high.conf", "high")
        count_low = self.serialize_file("uberAgent-ESA-am-threatcode-proc-creation-low.conf", "low")
        count_medium = self.serialize_file("uberAgent-ESA-am-threatcode-proc-creation-medium.conf", "medium")
