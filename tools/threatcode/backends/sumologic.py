...
# Copyright 2016-2018 Md Sulaiman, Florian Roth, juju4
...
from threatcode.parser.condition import ConditionOR, ThreatcodeAggregationParser
...
class SumoLogicBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into SumoLogic query. Contributed by SOC Prime. https://socprime.com"""
    ...
    def generate(self, threatcodeparser):
        try:
            self.product = threatcodeparser.parsedyaml['logsource']['product']   # OS or Software
        except KeyError:
            self.product = None
        try:
            self.service = threatcodeparser.parsedyaml['logsource']['service']   # Channel
        except KeyError:
            self.service = None
        try:
            self.category = threatcodeparser.parsedyaml['logsource']['category']   # Channel
        except KeyError:
            self.category = None
        # FIXME! don't get backend config mapping
        self.indices = threatcodeparser.get_logsource().index
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.threatcodeconfig.config.get("afl_fields"):
            self.keypresent = True
            aFL.extend(self.threatcodeconfig.config.get("afl_fields"))
        else:
            self.keypresent = False
        for item in self.threatcodeconfig.fieldmappings.values():
    ...
    # Skip logsource value from threatcode document for separate path.
        ...
        if key in self.allowedFieldsList:
            if key in ["_sourceCategory", "_sourceName"]:
                ...
                if key in ("LogName", "source"):
                    self.logname = value
                # need cleanValue if threatcode entry with single quote
...
class SumoLogicCSE(SumoLogicBackend):
    """Converts Threatcode rule into SumoLogic CSE query. Contributed by SOC Prime. https://socprime.com"""
...
class SumoLogicCSERule(SumoLogicCSE):
    """Converts Threatcode rule into SumoLogic CSE query"""
