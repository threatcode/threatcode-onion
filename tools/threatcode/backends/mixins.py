...
# Copyright 2016-2018 Md Sulaiman
...
class RulenameCommentMixin:
    ...
    def generateBefore(self, parsed):
        if self.rulecomment:
            try:
                return "%s%s\n" % (self.prefix, parsed.threatcodeParser.parsedyaml['title'])
            except KeyError:
                return ""
...
class MultiRuleOutputMixin:
    ...
    def getRuleName(self, threatcodeparser):
        """
        Generate a rule name from the title of the Threatcode rule with following properties:
        ...
        * Unique name by addition of a counter if generated name already in usage
        ...
        """
        try:
            rulename = threatcodeparser.parsedyaml["id"]
        except KeyError:
            rulename = threatcodeparser.parsedyaml["title"].replace(" ", "-").replace("(", "").replace(")", "")
