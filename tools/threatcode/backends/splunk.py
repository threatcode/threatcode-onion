...
# Copyright 2016-2018 Md Sulaiman, Florian Roth, Roey
...
class SplunkBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into Splunk Search Processing Language (SPL)."""
    identifier = "splunk"
    active = True
    index_field = "index"
...
class SplunkXMLBackend(SingleTextQueryBackend, MultiRuleOutputMixin):
    """Converts Threatcode rule into XML used for Splunk Dashboard Panels"""
    ...
    def generate(self, threatcodeparser):
        ...
        for parsed in threatcodeparser.condparsed:
            ...
            if query is not None:
                ...
                self.queries += threatcodeparser.parsedyaml.get("title") or ""
...
class CrowdStrikeBackend(SplunkBackend):
    """Converts Threatcode rule into CrowdStrike Search Processing Language (SPL)."""
    identifier = "crowdstrike"

    def generate(self, threatcodeparser):
        lgs = threatcodeparser.parsedyaml.get("logsource")
        if lgs.get("product") == "windows" and (lgs.get("service") == "sysmon" or lgs.get("category") == "process_creation"):
            fieldmappings = threatcodeparser.config.fieldmappings
            detections = threatcodeparser.definitions
            all_fields = dict()
            ...
            table_fields = threatcodeparser.parsedyaml.get("fields", [])
            res_table_fields = []
            for fl in table_fields:
                if fl in fieldmappings.keys():
                    res_table_fields.append(fl)
            threatcodeparser.parsedyaml["fields"] = res_table_fields
