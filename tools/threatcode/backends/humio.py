...
class HumioBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into Humio query. Contributed by SOC Prime. https://socprime.com"""
    identifier = "humio"
    active = True
    ...
    def generateMapItemNode(self, node):
        key, value = node
        if isinstance(value, ThreatcodeRegularExpressionModifier):# or isinstance(value, str) and "*" in value :
    ...
    def cleanValue(self, val):
        if isinstance(val, ThreatcodeRegularExpressionModifier):
            val = val.value
            if "\\" in val:
                val = re.sub(r"\\", r"\\\\\\", val)
