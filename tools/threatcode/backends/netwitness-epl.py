# NetWitness EPL output backend for threatcodec
# Copyright 2019 Tarik BOUDJEMAA (@snake-jump)
# Inspired from John Tuckner (@tuckner) NetWitness output backend for threatcodec

# NetWitness EPL backend for threatcodec uses netwitness-epl.yml config file
...
class NetWitnessEplBackend(SingleTextQueryBackend):
    """Converts Threatcode rule into RSA NetWitness EPL . Contributed by @snake-jump"""
    ...
    def generateMapItemNode(self, node):
        ...
        elif type(value) == ThreatcodeRegularExpressionModifier:  ## if value is regex
            regex = str(value)
    ...
    def generate(self, threatcodeparser):
        ...
        for parsed in threatcodeparser.condparsed:
            ...
            try:
                query=query.replace('RuleName', threatcodeparser.parsedyaml["title"].replace(" ",""))   # add rule name 
                query=query.replace('module_XXXXX', "module "+threatcodeparser.parsedyaml["title"].replace(" ","")) # add rule name
            except:
