...
temp:
  file.directory:
    - name: /opt/tc/conf/idh/http-skins
    - user: 939
    - group: 939
    - makedirs: True
...
opencanary_config:
  file.managed:
    - name: /opt/tc/conf/idh/opencanary.conf
...
tc-idh:
  docker_container.running:
    - image: {{ MANAGER }}:5000/{{ IMAGEREPO }}/tc-idh:{{ VERSION }}
    - name: tc-idh
    - detach: True
    - network_mode: host
    - binds:
      - /nsm/idh:/var/tmp:rw
      - /opt/tc/conf/idh/opencanary.conf:/etc/opencanaryd/opencanary.conf:ro
      - /opt/tc/conf/idh/http-skins:/usr/local/lib/python3.6/site-packages/opencanary/modules/data/http/skin:ro
    - watch:
      - file: opencanary_config
    - require:
      - file: opencanary_config

append_tc-idh_tc-status.conf:
  file.append:
