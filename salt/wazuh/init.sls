...
tc-wazuh:
  docker_container.running:
    - image: {{ MANAGER }}:5000/{{ IMAGEREPO }}/tc-wazuh:{{ VERSION }}
...
append_tc-wazuh_tc-status.conf:
  file.append:
...
registertheagent:
  cmd.run:
    - name: /usr/sbin/wazuh-register-agent
    - cwd: /
    - unless: ls /opt/tc/conf/wazuh/initial_agent_registration.log
...
hidsruledir:
 file.directory:
   - name: /opt/tc/rules/hids
   - user: 939
   - group: 939
   - makedirs: True

/opt/tc/rules/hids/local_rules.xml:
  file.symlink:
    - target: /nsm/wazuh/etc/rules/local_rules.xml

/opt/tc/rules/hids/ruleset:
  file.symlink:
