...
so-repo-sync:
  ...
  {%     endif %}
    - user: socore
    - name: '/usr/sbin/so-repo-sync >> /opt/tc/log/reposync/reposync.log 2>&1'
    - identifier: tc-repo-sync
...
strelkarepos:
  file.managed:
    - name: /opt/tc/conf/strelka/repos.txt
...
strelka-yara-update:
  ...
  {%       endif %}
    - user: socore
    - name: '/usr/sbin/so-yara-update >> /opt/tc/log/yarasync/yara-update.log 2>&1'
...
strelka-yara-download:
  ...
  {%       endif %}
    - user: socore
    - name: '/usr/sbin/so-yara-download >> /opt/tc/log/yarasync/yara-download.log 2>&1'
