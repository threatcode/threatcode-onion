...
so-influxdb:
  docker_container.running:
    ...
    - binds:
      ...
      - /opt/tc/conf/influxdb/config.yaml:/conf/config.yaml:ro
      - /opt/tc/conf/influxdb/etc:/etc/influxdb2:rw
...
influxdb-setup:
  cmd.run:
    - name: /usr/sbin/so-influxdb-manage setup &>> /opt/tc/log/influxdb/setup.log
    - require:
...
metrics_link_file:
  cmd.run:
    - name: tc-influxdb-manage dashboardpath "Security Onion Performance" > /opt/tc/saltstack/local/salt/influxdb/metrics_link.txt
    - require:
      - docker_container: tc-influxdb
...
get_influxdb_size:
  cron.present:
    - name: 'du -s -k /nsm/influxdb | cut -f1 > /opt/tc/log/telegraf/influxdb_size.log 2>&1'
