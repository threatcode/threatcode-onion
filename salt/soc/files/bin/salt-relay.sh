...
LOG_FILE=${LOG_FILE:-/opt/tc/log/soc/salt-relay.log}
PATH=${PATH}:/usr/sbin

# USE CAUTION when changing this value as all files in this dir will be deleted
QUEUE_DIR=/opt/tc/conf/soc/queue
  ...
  while [[ $tries -lt $max_tries ]]; do
    case "$op" in
      add)
        ...
        response=$(echo "$password" | tc-user "$op" --email "$email" --firstName "$firstName" --lastName "$lastName" --note "$note" --role "$role" --skip-sync)
        exit_code=$?
        ;;
      ...
      password)
        ...
        response=$(echo "$password" | tc-user "$op" --email "$email" --skip-sync)
        exit_code=$?
        ;;
      profile)
        email=$(echo "$request" | jq -r .email)
    ...
    tries=$((tries+1))
    if [[ "$response" == "Another process is using tc-user"* ]]; then
      log "Retrying after brief delay to let tc-user unlock ($tries/$max_tries)"
      sleep 5
    else
      break
    fi
