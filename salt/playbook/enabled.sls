...
so-playbook-sync_cron:
  cron.present:
    - name: /usr/sbin/so-playbook-sync > /opt/tc/log/playbook/sync.log 2>&1
    - identifier: tc-playbook-sync_cron
    - user: root
    - minute: '*/5'

so-playbook-ruleupdate_cron:
  cron.present:
    - name: /usr/sbin/so-playbook-ruleupdate > /opt/tc/log/playbook/update.log 2>&1
    - identifier: tc-playbook-ruleupdate_cron
    - user: root
    - minute: '1'
    - hour: '6'
