...
influxdb_continuous_query.present_patch:
  file.patch:
    ...
    - require:
      ...
      - pkg: patch_package
    - unless: ls /opt/tc/state/influxdb_continuous_query.py.patched

influxdb_continuous_query.py.patched:
  file.touch:
    - name: /opt/tc/state/influxdb_continuous_query.py.patched
    - onchanges:
...
influxdb_retention_policy.present_patch:
  file.patch:
    ...
    - require:
      - {{PYTHONINSTALLER}}: python3_influxdb
      - pkg: patch_package
    - unless: ls /opt/tc/state/influxdb_retention_policy.py.patched

influxdb_retention_policy.py.patched:
  file.touch:
    - name: /opt/tc/state/influxdb_retention_policy.py.patched
    - onchanges:
...
influxdbmod.py_shard_duration_patch:
  file.patch:
    ...
    - require:
      - {{PYTHONINSTALLER}}: python3_influxdb
      - pkg: patch_package
    - unless: ls /opt/tc/state/influxdbmod.py.patched

influxdbmod.py.patched:
  file.touch:
    - name: /opt/tc/state/influxdbmod.py.patched
    - onchanges:
