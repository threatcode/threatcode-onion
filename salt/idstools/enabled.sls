...
so-rule-update:
  cron.present:
    - name: /usr/sbin/so-rule-update > /opt/tc/log/idstools/download_cron.log 2>&1
    - identifier: tc-rule-update
    - user: root
    - minute: '1'
    - hour: '7'

# order this last to give tc-idstools container time to be ready
run_so-rule-update:
  cmd.run:
    - name: '/usr/sbin/so-rule-update > /opt/tc/log/idstools/download_idstools_state.log 2>&1'
    - require:
      - docker_container: tc-idstools
    - onchanges:
      - file: idstoolsetcsync
