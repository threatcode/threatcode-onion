...
function add_filter_file()
	...
	#if ( ThreatCode::sensorname != "" )
	#	real_filter_filename = gsub(real_filter_filename, /\{\{sensorname\}\}/, ThreatCode::sensorname);

	# Support the interface template value.
	#if ( ThreatCode::interface != "" )
	#	real_filter_filename = gsub(real_filter_filename, /\{\{interface\}\}/, ThreatCode::interface);
...
#event ThreatCode::found_sensorname(name: string)
#	{
#	add_filter_file();
#	}
